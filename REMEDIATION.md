...
| C3 | **Backup code MFA rotto nell'endpoint standalone** — `code in backup_codes.codes` fallisce perché i codici sono hash bcrypt.                                     | `authglow/api/mfa.py` (verify_mfa_login)                                                                | Usare `mfa_service.verify_user_backup_code()` come fatto correttamente in `auth.py`.                                     | pending |      |
