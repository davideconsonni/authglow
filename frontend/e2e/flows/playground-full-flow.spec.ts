...
test.describe('Playground — Create Client → Authorize → Introspect → Revoke', () => {
  ...
  test.afterAll(async ({ page }) => {
    if (clientId) {
      ...
      if (token) {
        await page.request.delete(`${API_URL}/api/oauth-clients/${clientId}`, {
  ...
  test('full OAuth2 playground flow: create client → authorize → introspect → revoke', async ({ page }) => {
    // ── Step 1: Create an OAuth2 client via API ──
    ...
    const createResp = await page.request.post(`${API_URL}/api/oauth-clients`, {
      data: {
    ...
    // Fill introspection form via UI
    ...
    // Verify introspection shows active token
    ...
    // Token should be auto-filled from the store, click Next
    ...
    // Verify revocation succeeded (200 badge visible)
