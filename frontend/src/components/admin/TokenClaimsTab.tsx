// Visual Token Builder — per-OAuth2-client custom data editor.
//
// Design principles:
//
// 1. **Token as a concrete object**: the admin sees the token
//    as a visual card with blocks for each field, not an
//    abstract "claim policy".
//
...
export function TokenClaimsTab({ clientId, clientName, onClose }: TokenClaimsTabProps) {
  ...
  const handleSave = async () => {
    ...
    setError(null)
    try {
      await api.put(`/api/admin/oauth-clients/${encodeURIComponent(clientId)}/claim-policy`, { rules: draft })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-8" data-testid="claim-policy-modal">
      ...
      <div className="relative z-10 flex w-full max-w-3xl flex-col rounded-2xl border border-surface-2 bg-bg-primary shadow-glow-accent max-h-[calc(100vh-4rem)]">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          ...
          {error && (
            <div className="mb-4">
              <Banner variant="error" sticky onDismiss={() => setError(null)}>
                {error}
              </Banner>
            </div>
          )}
                ...
                {ruleCount === 0 && formMode === 'closed' ? (
                  <div className="rounded-xl border border-dashed border-surface-2 bg-bg-primary p-6 text-center" data-testid="claim-policy-empty-state">
                    ...
                    <p className="mt-0.5 text-[10px] text-text-muted">
                      The token only has the standard fields above. Add custom data like tenant ID or subscription level.
                    </p>
                    <button
                      onClick={openAddForm}
                ...
                <div className="mt-3 rounded-xl border border-brand-accent/30 bg-bg-primary p-4 space-y-3">
                  ...
                  <div>
                    <div className="mb-1.5 flex items-center gap-1.5">
                      ...
                      <HelpTooltip text="Choose where the system should look to find this data. Most common: pull it from the user's profile." />
                    </div>
                  ...
                  <div>
                    <div className="mb-1 flex items-center gap-1.5">
                      ...
                      <HelpTooltip text="Leave empty to always include this data. Type a scope name to only include it when the user approves that permission." />
                    </div>
                    <input
