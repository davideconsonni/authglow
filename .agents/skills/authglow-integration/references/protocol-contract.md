...
`/api/token` is not registered. The AuthGlow dashboard uses the same Authorization Code + PKCE flow as other public clients (tokens via the discovered `token_endpoint`); third-party clients must use their own registered client and discovery metadata.
