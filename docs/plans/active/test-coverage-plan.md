...
- [x] COV-BE-005: `backend/authglow/api/federation.py` — 46.1% → 100% (272/272 statement, tutti i branch). Nuovo `tests/unit/test_federation_api.py` (49 test: login/guards/callback completo/bridge OAuth2/consent/CRUD admin). Vedi Diario.
- [x] COV-BE-006: `backend/authglow/api/phone_verification.py` — 48.6% → 100% (35/35 statement, tutti i branch). Nuovo `tests/unit/test_phone_verification_api.py` (5 test: request/verify success+failure + factory). Vedi Diario.
- [x] COV-BE-007: `backend/authglow/api/password_reset.py` — 49.6% → 100% (135/135 statement, tutti i branch). Nuovo `tests/unit/test_password_reset_api.py` (23 test: request/confirm/admin CRUD; `change_expired_password` già coperto via HTTP altrove). Vedi Diario.
