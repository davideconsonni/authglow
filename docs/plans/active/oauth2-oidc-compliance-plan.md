...
- [x] **[OA-306]** 3.6 Done fase: un solo code-path per grant, first-party = client normale + cookie.
...
- [x] **[OA-402]** 4.2 MUST rimasti: `nonce` (obbligatorio con `openid`+code? oggi copiato se presente ma non imposto),
...
- [x] **[OA-403]** 4.3 Snapshot: discovery + JWKS (rotazione kid, `304`/ETag, revoked esclusi — `api/oidc.py` ~L160-243)
...
| 2026-09-12 | OA-403 | Fase 4 / 4.3 Snapshot discovery/JWKS/userinfo | OA-403 (questo commit) | Solo test + micro-refactor (filtro kid in `_publishable_kids`): foto esatta 32 campi discovery, revocate escluse dalla JWKS, userinfo senza `aud` → 401. Già coperti citati (ETag/304, rotazione, openid, DPoP). Area 48 passed + matrice 35/35; ruff/mypy puliti. |
