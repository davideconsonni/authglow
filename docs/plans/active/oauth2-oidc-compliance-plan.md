...
| 2026-09-12 | OA-301 | Fase 3 / 3.1 Token first-party standard | OA-301 (questo commit) | `api/auth.py`: cancellato ramo `{"ok": True}` (cookie già su risposta a L1484) → sempre `Token` + cookie; `OAuthCallbackPage.tsx`: parsing Token-unico + nonce se richiesto; gate rafforzato (access/refresh/id_token + 2 cookie); cookie-flow `/api/auth/refresh` invariato. Matrice 28 passed + area 73 passed; tsc/eslint + unit frontend verdi; E2E login-dashboard non gateabile (fallimento pre-esistente a login-submit, provato via stash). |
