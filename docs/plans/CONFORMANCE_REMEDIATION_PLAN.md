...
- [x] **T.3** Considerare `sender-constrained` tokens (DPoP, RFC 9449) per FAPI 2.0.
  - **Done**: DPoP implementato opt-in per client (`dpop_bound: bool`). Token endpoint richiede un DPoP proof JWT firmato ES256 (jwk embedded nell'header) per client DPoP-bound. Token response ha `token_type=DPoP` e claim `cnf.jkt` (RFC 7800 + RFC 9449 §4.2). UserInfo verifica il proof su token DPoP-bound (controlla htm=GET, htu=userinfo URL, ath=SHA-256 del token, replay protection su jti). Discovery annuncia `dpop_signing_alg_values_supported=[ES256]`. Replay JTI riusa `jti_cache` con namespace `dpop:`. 22 unit + 10 integration test passano (32 nuovi); ruff+mypy clean (1 errore pre-esistente non mio in federation.py:636); 333 frontend test passano (4 nuovi AdminOAuthClientsPage + 2 nuovi phase-audit). UI admin: toggle DPoP-bound + badge "DPoP" nella tabella + hint ES256 key pair.
- [x] **T.4** Documentare le differenze rispetto a FAPI 2.0 in `docs/FAPI.md`.
  - **Done**: creato `docs/FAPI.md` (~400 righe) con gap analysis dettagliato rispetto a FAPI 2.0 Security Profile (Part 2) e FAPI 1.0 Advanced. 9 tabelle di conformità coprono Client Registration, Authorization Request, Authorization Response, Token Endpoint, Refresh Token, Sender-Constrained Tokens, ID Token, UserInfo, Discovery, Logout, Error Responses. 4 nuovi test in `frontend/tests/phase-audit.test.ts` verificano esistenza del file + presenza delle sezioni chiave (Conformance Matrix, Gap Roadmap, PAR, DPoP, mTLS, T.2 client_secret_jwt/private_key_jwt, T.3 RFC 9449/ES256/cnf). 48/48 test phase-audit passano. Gap principali documentati: PAR (P1), JARM (P2), mTLS (P2), code lifetime 30s (P2 config-only).
