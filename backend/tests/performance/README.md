...
Micro-benchmarks for AuthGlow hot-path components.

Unlike `tests/unit/` (fast, deterministic, runs on every commit)
and `tests/integration/` (cross-module flows), this suite:
...
`hash_password_async` / `verify_password_async`; the suite is the
