...
class TestValidateUpdates:
    async def test_accepts_matching_scalars(self, test_settings, stub_repo):
        service = SettingsOverrideService(repository=stub_repo, settings=test_settings)
...
class TestSetAndApply:
    ...
    async def test_apply_overrides_sets_live_attribute(self, test_settings, stub_repo):
        service = SettingsOverrideService(repository=stub_repo, settings=test_settings)
    ...
    async def test_apply_overrides_skips_invalid_entries(self, test_settings, stub_repo):
        service = SettingsOverrideService(repository=stub_repo, settings=test_settings)
...
class TestRefreshIfChanged:
    ...
    async def test_noop_when_unchanged_after_mark_synced(self, test_settings, stub_repo):
        service = SettingsOverrideService(repository=stub_repo, settings=test_settings)
...
class TestRemoveOverride:
    async def test_removes_and_restores_pristine(self, test_settings, stub_repo):
        ...
        service = SettingsOverrideService(repository=stub_repo, settings=test_settings)
        # ``set_overrides`` persists only; the live application is
        # ``apply_overrides`` (the API PATCH handler calls them in this
        # exact order).
    ...
    async def test_remove_without_pristine_snapshot_skips_restore(self, test_settings, stub_repo):
        service = SettingsOverrideService(repository=stub_repo, settings=test_settings)
        await service.set_overrides({"app_name": "Renamed"})
    ...
    async def test_refresh_noop_after_removal_mark_synced(self, test_settings, stub_repo):
        capture_pristine(test_settings)
        service = SettingsOverrideService(repository=stub_repo, settings=test_settings)
        await service.set_overrides({"app_name": "Renamed"})
        await service.remove_override("app_name")
