"""Unit tests for the File-backed admin-action repository.

Covers ``FileAdminActionRepository``. The service-level behaviour is
exercised implicitly by the integration tests in
``tests/integration/test_admin_api.py`` (which mock the service).
There is no dedicated ``tests/unit/test_admin_action.py`` because
``AdminActionService`` is a thin pass-through (record + list_for_user
...
Regression focus: the pre-refactor ``AdminActionService`` had two
...
class TestFileAdminActionRepository:
    ...
    def test_subdir_layout(self, test_settings):
        ...
        assert repo._subdir == "admin_actions"
        assert Path(repo._storage_path).name == "admin_actions"

    def test_satisfies_protocol(self, test_settings):
        repo = self._make_repo(test_settings)
        assert isinstance(repo, AdminActionRepository)
    ...
    async def test_record_creates_user_subdir(self, test_settings):
        ...
        await repo.record(
            ...
            target_user_id="target-rt",
        )
        user_dir = Path(repo._user_dir("target-rt"))
        assert user_dir.exists()
        assert user_dir.is_dir()

    async def test_record_writes_action_file(self, test_settings):
        repo = self._make_repo(test_settings)
        await repo.record(
            admin_user_id="admin-1",
            admin_email="admin@example.com",
            action_type="user.suspend",
            target_user_id="target-w",
        )
        # Discover the new action by listing the user dir
        user_dir = Path(repo._user_dir("target-w"))
        files = list(user_dir.glob("*.json"))
        assert len(files) == 1
        raw = files[0].read_text()
        assert "admin-1" in raw
        assert "target-w" in raw
        assert "user.suspend" in raw

    async def test_record_generates_id(self, test_settings):
        repo = self._make_repo(test_settings)
        # No way to read the id back from record() (returns None),
        # so we use list_for_user to verify a UUID-shaped id was
        # generated.
        await repo.record(
            admin_user_id="admin-1",
            admin_email="admin@example.com",
        ...
        assert len(action_id) >= 32  # UUID4 with hyphens = 36 chars
    ...
    async def test_record_preserves_optional_fields(self, test_settings):
        ...
        await repo.record(
            admin_user_id="admin-1",
            admin_email="admin@example.com",
            action_type="user.role.assign",
            target_user_id="target-opts",
            target_user_email="target@example.com",
            details={"role": "admin", "duration_days": 30},
            ip_address="10.0.0.42",
        )
        page, total = await repo.list_for_user("target-opts")
        assert total == 1
        record = page[0]
        assert record["target_user_email"] == "target@example.com"
        assert record["details"] == {"role": "admin", "duration_days": 30}
    ...
    async def test_record_uses_ensure_parent_not_direct_os_makedirs(self, test_settings):
        ...
        with patch.object(repo, "_ensure_parent", wraps=repo._ensure_parent) as spy_ensure_parent:
            await repo.record(
                ...
                admin_email="admin@example.com",
                action_type="user.suspend",
                target_user_id="target-makedirs",
            )
    ...
    async def test_list_for_user_returns_records_newest_first(self, test_settings):
        ...
        # record 3 actions in known order
        for i, ts in enumerate(
            ...
            for f in user_dir.glob("*.json"):
                ...
                if payload.get("admin_user_id") == admin_id:
                    payload["id"] = f"ord-{i}"
    ...
    async def test_list_for_user_pagination(self, test_settings):
        ...
        for i in range(5):
            admin_id = f"admin-page-{i}"
            await repo.record(
                admin_user_id=admin_id,
                admin_email="admin@example.com",
                action_type="user.suspend",
                target_user_id="target-page",
            )
    ...
    async def test_list_for_user_filters_by_target_user(self, test_settings):
        repo = self._make_repo(test_settings)
        ...
        assert total == 1
        assert page[0]["target_user_id"] == "target-a"
    ...
    async def test_list_for_user_skips_corrupt_json(self, test_settings):
        ...
        await repo.record(
            ...
            admin_email="admin@example.com",
            action_type="user.suspend",
            target_user_id="target-corrupt",
        )
        page, total = await repo.list_for_user("target-corrupt")
        assert total == 1
        assert page[0]["target_user_id"] == "target-corrupt"
...
class TestFileAdminActionRepositoryWithPatchedSettings:
