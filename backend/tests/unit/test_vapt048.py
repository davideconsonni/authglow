...
class TestVapt048LoginForAccessTokenOrdering:
    """The ``/api/token`` endpoint must follow the same order."""

    def test_lockout_check_precedes_bcrypt_in_login_for_access_token(self):
        from authglow.api.auth import login_for_access_token

        source = inspect.getsource(login_for_access_token)
        lockout_pos = source.find("is_account_locked")
        verify_pwd_pos = source.find("verify_and_maybe_rehash_password")
        assert lockout_pos != -1
        assert verify_pwd_pos != -1
        assert lockout_pos < verify_pwd_pos, (
            "VAPT-048 regression: login_for_access_token must check "
            "is_account_locked BEFORE the bcrypt compare."
        )
...
class TestVapt048LoginForAccessTokenLockedUser:
    """``/api/token`` behavioural coverage: the 423 path must not
    touch bcrypt, the 401 path must record the failed login."""

    def test_locked_user_gets_423_without_bcrypt(self, test_settings):
        ...
        storage.is_account_locked = AsyncMock(return_value=True)
        storage.record_failed_login = AsyncMock()
        storage.reset_failed_login_attempts = AsyncMock()
        bcrypt_spy = MagicMock(return_value=(True, None))
        storage.verify_and_maybe_rehash_password = bcrypt_spy

        from authglow.api.auth import get_audit_service, get_user_storage
    ...
    def test_non_locked_user_with_wrong_password_records_failed(self, test_settings):
        from authglow.api.auth import router
        from authglow.models.user import User
        from authglow.services.password import hash_password
        ...
        assert response.status_code == 401, response.text
        storage.verify_and_maybe_rehash_password.assert_awaited_once()
        storage.record_failed_login.assert_awaited_once_with("u-active")
